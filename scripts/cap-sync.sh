#!/usr/bin/env bash
# Wrapper around `npx cap sync` that re-applies Firebase iOS SDK to
# CapApp-SPM/Package.swift, since `cap sync` overwrites that file every run.
# Run this instead of `npx cap sync` whenever syncing native projects.
...
if [ -f "$PACKAGE_SWIFT" ] && ! grep -q "firebase-ios-sdk" "$PACKAGE_SWIFT"; then
  ...
  # Add firebase-ios-sdk package after the last .package(name: ...) entry
  /usr/bin/sed -i '' \
