...
export default function OfferManager() {
  ...
  const { data: affiliatesData } = useQuery({ queryKey:["my-affiliates"], queryFn:()=>api.get("/affiliates").then(r=>r.data) });
  const { data: coproducersData } = useQuery({ queryKey:["coproducers-list"], queryFn:()=>api.get("/coproducers").then(r=>r.data) });
  ...
  return (
    <div>
      ...
      <Modal open={!!splitOfferId} onClose={()=>setSplitOfferId(null)} title="Configurar Splits" size="lg"
        ...
        {/* Banner fixo da plataforma — não editável */}
        ...
        {/* Splits configuráveis — os 95% restantes */}
