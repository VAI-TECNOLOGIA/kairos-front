  ...
  return (
    <div>
      ...
      <Modal open={!!coprodOffer} onClose={() => setCoprodOffer(null)} title={`Co-produção — ${coprodOffer?.name || ''}`}
        footer={
          coprodOffer ? (() => {
            const rules = coprodOffer.splitRules || [];
            const platformBps = rules.find((r: any) => r.recipientType === 'PLATFORM')?.basisPoints || 0;
        ...
        {coprodOffer && (() => {
          const rules = coprodOffer.splitRules || [];
          const platformBps = rules.find((r: any) => r.recipientType === 'PLATFORM')?.basisPoints || 0;
          const namedCoprods = rules.filter((r: any) => r.recipientType === 'COPRODUCER' && r.recipientId);
